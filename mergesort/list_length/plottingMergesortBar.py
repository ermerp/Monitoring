...
for ALGORITHM, csv_file, color in zip(algorithms, csv_files, colors):
    ...
    data['list_length'] = pd.to_numeric(data['list_length'], errors='coerce')
    ...
    max_values = data.groupby('list_length').agg({
        'cpu_usage': 'max',
        'memory_usage': 'max',
        'num_threads': 'max',
    }).reset_index()

    mean_values = data.groupby('list_length').agg({
        'cpu_usage': 'mean',
        'memory_usage': 'mean',
        'num_threads': 'mean',
    }).reset_index()
...
time_data['list_length'] = pd.to_numeric(time_data['list_length'], errors='coerce')
...
def plot_metric_bar(metric, ylabel, filename, log_y=False):
    ...
    for i, (ALGORITHM, color) in enumerate(zip(algorithms, colors)):
        ...
        # Positionen auf der X-Achse für die Balken (log10 von list_length)
        positions = np.log10(max_values['list_length']) + (i - (num_algorithms - 1) / 2) * width
    ...
    # Setzen der x-Ticks auf die logarithmischen Werte der list_length
    list_lengths = [10000, 100000, 1000000, 10000000]
    plt.xticks(np.log10(list_lengths), list_lengths)

    # Setzen der y-Achse auf logarithmisch (log10), falls gewünscht
    if log_y:
        plt.yscale('log')
...
# Plot für CPU Usage als Säulendiagramm erstellen (mit linearer y-Achse)
plot_metric_bar("cpu_usage", "CPU-Auslastung (%)", "cpu_usage_bar_plot.png", log_y=False)

# Plot für Memory Usage als Säulendiagramm erstellen (mit logarithmischer y-Achse)
plot_metric_bar("memory_usage", "Arbeitsspeicherverbrauch (MB)", "memory_usage_bar_plot.png", log_y=True)

# Plot für die Anzahl der Threads als Säulendiagramm erstellen (mit linearer y-Achse)
plot_metric_bar("num_threads", "Thread-Anzahl", "num_threads_bar_plot.png", log_y=False)


# Funktion für das Plotten der Ausführungszeiten als Säulendiagramm
def plot_execution_time_bar():
    ...
    for i, (ALGORITHM, color) in enumerate(zip(algorithms, colors)):
        # Positionen auf der X-Achse für die Balken (log10 von list_length)
        positions = np.log10(time_data['list_length']) + (i - (num_algorithms - 1) / 2) * width
    ...
    # Setzen der y-Achse auf logarithmisch (log10)
    plt.yscale('log')
...
# Plot für Ausführungszeit als Säulendiagramm erstellen (mit logarithmischer y-Achse)
